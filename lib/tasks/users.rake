namespace :users do
  ...
  task :create_groups => :environment do
    ...
    Group.create(key: 'managers', title: 'Ведущие менеджеры') if Group.find_by(key: 'managers').nil?
    Group.create(key: 'settlers', title: 'Mенеджеры') if Group.find_by(key: 'settlers').nil?
  ...
  #  unless admin = User.find_by(email: '-')
  #    admin = User.new(email: '-', password: '-', password_confirmation: '-')
